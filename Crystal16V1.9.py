...
class Crystal16:
    def __init__(self, port, baudrate=9600):
        self.all_data, self.RBs, self.total_time_min,\
            self.version, self.date_time= pd.DataFrame([]), [], [0,0,0,0], [], [] # Initialize global variables
        self.sample_rate=50 # Set sample rate for writing data to the csv
        self.n_max_steps= 15 # Set max length of recipe steps writen at once in each block, limited by the Crystal16 V1 memory
        ...
        print("Enter directory of recipe files for each block.")
    ...
    def save_temperature_profile(self, RB, recipe_matrix):
        ...
        RBs= {'0':'A', '1':'B', '2':'C', '3':'D'}
        os.chdir(self.save_directory) # Set working directory to save the .csv file
        filename=f'Recipe_{self.filename}_{RBs[RB]}.csv'
        ...
        try:
            ...
            filename=f'Autosave_Recipe_{self.filename}_{RBs[RB]}.csv'
...
#                              |___/        
 ...
 #                     |_|                                                        |___/                                                            
    ...
    def make_recipes(self, RB:str):
        ...
        a,b= DeltaT[RB]
        df = pd.read_csv(self.recipe_directory+'\Recipe_'+ReactorBlocks[int(RB)] + '.csv')        
    ...
    def run_thread(self, RB, times_batches, commands_batches):        
            for i, batch in enumerate(commands_batches):
                ...
                if i == 0:
                    sleep_duration = times_batches[i][-1]
                else:
                    sleep_duration = times_batches[i][-1] - times_batches[i - 1][-1]

                with self.lock:
                    time.sleep(1)
                    self.stop_recipe(RB)
                    time.sleep(1)
                    self.clear_recipe(RB)
                    ...
                    time.sleep(1)
                    self.start_recipe(RB)
                    print(f"Block {RB}: Sleeping for {format(sleep_duration, '.1f')} minutes")
                
                time.sleep(sleep_duration * 60 + 1)
    ...
    def break_load_and_start_recipe(self, RBs):
        ...
        for RB in RBs:
                ...
                total_times_batches = [total_times_RB[i:i+self.n_max_steps] for i in range(0, len(total_times_RB), self.n_max_steps)]
                recipe_commands_batches = [recipe_commands_RB[i:i+self.n_max_steps] for i in range(0, len(recipe_commands_RB), self.n_max_steps)]

                block_thread = threading.Thread(target=self.run_thread, args=(RB, total_times_batches, recipe_commands_batches))
